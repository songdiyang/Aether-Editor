...
use crate::remote_fs::{RemoteFs, FsEvent, Result};
...
impl RemoteWorkspace {
    ...
    pub fn save_file(&mut self, remote_path: &str) -> Result<()> {
        ...
        // 递增版本
        let version = self.file_versions.entry(remote_path.to_string()).or_insert(0);
        *version += 1;
    ...
    pub fn sync_tree(&self, remote_path: &str) -> Result<()> {
        ...
        for entry in entries {
            ...
            let local_path = self.local_cache.join(remote_full_path.trim_start_matches('/'));
