    ...
    fn lex_next(&self, bytes: &[u8], pos: usize) -> (LexemeSpan, usize) {
        ...
        match ch {
            ...
            b'[' => {
                ...
                (LexemeSpan { start: pos, len: i - pos, kind: TokenKind::TomlTable, flags: 0 }, i)
            }
            b'"' => {
            ...
            b'a'..=b'z' | b'A'..=b'Z' | b'_' => {
                ...
                let kind = if is_key { TokenKind::JsonKey } else { TokenKind::Identifier };
                (LexemeSpan { start: pos, len: end - pos, kind, flags: 0 }, end)
            }
            b'=' | b'.' | b',' => {
...
fn skip_number_or_date(bytes: &[u8], pos: usize) -> usize {
    let mut i = pos;
    while i < bytes.len() && (bytes[i].is_ascii_digit() || bytes[i] == b'-' || bytes[i] == b':' || bytes[i] == b'T' || bytes[i] == b'Z' || bytes[i] == b'+' || bytes[i] == b'.' || bytes[i] == b'e' || bytes[i] == b'E') {
        i += 1;
    }
    i
}
...
mod tests {
    ...
    fn test_toml_table() {
        ...
        let table_count = tokens.iter().filter(|t| t.kind == TokenKind::TomlTable).count();
    ...
    fn test_toml_keys() {
        ...
        let key_count = tokens.iter().filter(|t| t.kind == TokenKind::JsonKey).count();
