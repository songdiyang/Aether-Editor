...
use crate::types::*;
use crate::transport::{DapTransport, spawn_adapter};
...
impl DebugSession {
    /// 启动并初始化调试会话
    pub async fn start(config: AdapterConfig, event_tx: mpsc::UnboundedSender<DapEventUi>) -> std::io::Result<Self> {
    ...
    /// 启动调试（launch）
    pub async fn launch(&mut self, program: &str, args: Vec<String>, cwd: Option<&str>) -> std::io::Result<()> {
        let seq = self.seq_generator.next();
    ...
    /// 设置断点
    pub async fn set_breakpoints(&mut self, source_path: &str, lines: Vec<u32>) -> std::io::Result<Vec<Breakpoint>> {
        let seq = self.seq_generator.next();
        let breakpoints: Vec<serde_json::Value> = lines.iter().map(|&line| {
            ...
            match message {
                DapMessage::Response(resp) if resp.command == "setBreakpoints" && resp.success => {
                    ...
                    let breakpoints: Vec<Breakpoint> = serde_json::from_value(
                        body.get("breakpoints").cloned().unwrap_or(serde_json::Value::Array(vec![]))
                    ).unwrap_or_default();
    ...
    pub async fn continue_execution(&mut self, thread_id: i64) -> std::io::Result<()> {
        self.send_simple_request("continue", serde_json::json!({"threadId": thread_id})).await
    }

    /// 单步跳过（Next）
    pub async fn next(&mut self, thread_id: i64) -> std::io::Result<()> {
        self.send_simple_request("next", serde_json::json!({"threadId": thread_id})).await
    }

    /// 单步进入（Step In）
    pub async fn step_in(&mut self, thread_id: i64) -> std::io::Result<()> {
        self.send_simple_request("stepIn", serde_json::json!({"threadId": thread_id})).await
    }

    /// 单步跳出（Step Out）
    pub async fn step_out(&mut self, thread_id: i64) -> std::io::Result<()> {
        self.send_simple_request("stepOut", serde_json::json!({"threadId": thread_id})).await
    }

    /// 暂停
    pub async fn pause(&mut self, thread_id: i64) -> std::io::Result<()> {
        self.send_simple_request("pause", serde_json::json!({"threadId": thread_id})).await
    }
            ...
            match message {
                DapMessage::Response(resp) if resp.command == "stackTrace" && resp.success => {
                    ...
                    let frames: Vec<StackFrame> = serde_json::from_value(
                        body.get("stackFrames").cloned().unwrap_or(serde_json::Value::Array(vec![]))
                    ).unwrap_or_default();
                    return Ok(frames);
                }
                DapMessage::Event(evt) => {
            ...
            match message {
                DapMessage::Response(resp) if resp.command == "scopes" && resp.success => {
                    ...
                    let scopes: Vec<Scope> = serde_json::from_value(
                        body.get("scopes").cloned().unwrap_or(serde_json::Value::Array(vec![]))
                    ).unwrap_or_default();
                    return Ok(scopes);
                }
                DapMessage::Event(evt) => {
            ...
            match message {
                DapMessage::Response(resp) if resp.command == "variables" && resp.success => {
                    ...
                    let variables: Vec<Variable> = serde_json::from_value(
                        body.get("variables").cloned().unwrap_or(serde_json::Value::Array(vec![]))
                    ).unwrap_or_default();
                    return Ok(variables);
                }
                DapMessage::Event(evt) => {
    ...
    /// 评估表达式
    pub async fn evaluate(&mut self, expression: &str, frame_id: Option<i64>) -> std::io::Result<String> {
        let seq = self.seq_generator.next();
    ...
    pub async fn disconnect(&mut self) -> std::io::Result<()> {
        self.send_simple_request("disconnect", serde_json::json!({"terminateDebuggee": true})).await?;
    ...
    /// 发送简单请求（不需要解析响应体）
    async fn send_simple_request(&mut self, command: &str, arguments: serde_json::Value) -> std::io::Result<()> {
        let seq = self.seq_generator.next();
        let request = DapMessage::Request(DapRequest {
            seq,
            message_type: "request".to_string(),
        ...
        match event.event.as_str() {
            "stopped" => {
                ...
                let thread_id = event.body.as_ref()
                    ...
                    .and_then(|v| v.as_i64());
                let _ = self.event_tx.send(DapEventUi::Stopped { reason, thread_id });
            }
            "continued" => {
                self.state = DebugSessionState::Running;
                let thread_id = event.body.as_ref()
            ...
            "exited" => {
                let exit_code = event.body.as_ref()
            ...
            "breakpoint" => {
                if let Some(body) = &event.body {
                    if let Ok(bp) = serde_json::from_value::<Breakpoint>(body.clone()) {
                        let _ = self.event_tx.send(DapEventUi::BreakpointValidated { breakpoint: bp });
                    }
                }
            }
            _ => {}
