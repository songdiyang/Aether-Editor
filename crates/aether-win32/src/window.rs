...
/// WM_LBUTTONDOWN
unsafe fn on_l_b_u_t_t_o_n_d_o_w_n(
    hwnd: HWND,
    _msg: u32,
    _wparam: WPARAM,
    lparam: LPARAM,
    ...
    if let Some(state) = get_and_set_state(hwnd) {
        ...
        if st.activity_bar.customize_mode && !_activity_region_lp.contains(mouse_x, mouse_y) {
        ...
        if titlebar_region.contains(mouse_x, mouse_y) {
            ...
            let user_btn_y = titlebar_region.y + (titlebar_region.height - user_btn_size) / 2.0;
                    ...
                    if let Some(item) = st.menu_bar.items.get(active_idx) {
                        if let Some(menu_item) = item.items.get(sub_idx) {
                            if menu_item.enabled
                                && menu_item.command_id != crate::menu_bar::CommandId::None
                            {
...
/// WM_LBUTTONDBLCLK
unsafe fn on_l_b_u_t_t_o_n_d_b_l_c_l_k(
    hwnd: HWND,
    _msg: u32,
    _wparam: WPARAM,
    lparam: LPARAM,
...
/// WM_NCACTIVATE
unsafe fn on_n_c_a_c_t_i_v_a_t_e(
    _hwnd: HWND,
    _msg: u32,
    _wparam: WPARAM,
    _lparam: LPARAM,
...
/// WM_NCCALCSIZE
unsafe fn on_n_c_c_a_l_c_s_i_z_e(
    _hwnd: HWND,
    _msg: u32,
    _wparam: WPARAM,
    _lparam: LPARAM,
...
/// WM_ERASEBKGND
unsafe fn on_e_r_a_s_e_b_k_g_n_d(
    _hwnd: HWND,
    _msg: u32,
    _wparam: WPARAM,
    _lparam: LPARAM,
                        ...
                        match focus {
                            crate::editor::FindReplaceFocus::FindQuery => {
                                ...
                                if !state.borrow().find_results.is_empty() {
                                    ...
                                    state.borrow_mut().selection_end =
...
/// WM_MOUSEHWHEEL
unsafe fn on_m_o_u_s_e_h_w_h_e_e_l(
    hwnd: HWND,
    _msg: u32,
    wparam: WPARAM,
    lparam: LPARAM,
